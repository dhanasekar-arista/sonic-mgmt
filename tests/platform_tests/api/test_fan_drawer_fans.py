...
File: test_fan_drawer_fans.py
=============================================================================

Description:
    Validates Platform API functionality for Fan objects within FanDrawer containers.
    Tests fan metadata, speed control, direction detection, and LED management for
    fans organized in drawer-based cooling systems. Stops thermal control daemon
    during testing to enable manual speed control.

Test Intent:
    - test_get_name: Verify fan name retrieval and match with platform.json
    - test_get_presence: Confirm all fan drawer fans are present
    - test_get_model: Validate fan model string retrieval
    - test_get_serial: Verify fan serial number retrieval
    - test_get_status: Test fan operational status
    ...
    - test_get_fans_target_speed: Test target speed setting and retrieval per drawer
    - test_set_fans_speed: Verify fan speed control with tolerance validation
    - test_set_fans_led: Test LED color control (fault/normal/off) per fan

Topology:
    Any topology with fan drawer support (typically modular chassis)

Fixtures Used:
    - platform_api_conn: Platform API service connection
    - setup: Function-scoped autofixture managing thermal control daemon lifecycle
    - duthosts: Multi-DUT host fixture
    - enum_rand_one_per_hwsku_hostname: Selects one DUT per hardware SKU
    - suspend_and_resume_hw_tc_on_mellanox_device: Mellanox-specific thermal control suspension

Dependencies:
    - tests.common.helpers.platform_api (chassis, fan_drawer, fan_drawer_fan)
    - platform.json (fan drawer and fan metadata)
    - thermal_control_daemon (stopped during tests, restarted after)

Notes:
    - Test skips if no fan drawers found on device
    - Skips on older images (201811, 201911) lacking fan_drawer API
    - Speed control tests skip fans marked non-controllable in platform.json
    - Speed changes validated against is_under_speed/is_over_speed tolerance
    - LED tests skip if LED not available/controllable or lacks supported colors
    - Mellanox platforms don't support 'off' LED color
    - Test respects platform-specific speed minimum/maximum/delay per drawer/fan
    - Entire drawer skipped if all fans non-controllable
=============================================================================
"""
import logging
import random

    ...
    with allure.step("Verifying port serdes expected attributes in COUNTERS_DB PORT_PHY_ATTR table."):
        for port_serdes_oid, (asic, port_id) in port_serdes_info_map.items():
            try:
                ...
                pattern = r'\[\{(tap[0-9]+):(?:-?[0-9]+(?:,-?[0-9]+)*)?\}(?:,\{(tap[0-9]+):(?:-?[0-9]+(?:,-?[0-9]+)*)?\})*\]'
                match = re.match(pattern, tx_fir_taps)
                pytest_assert(match, "tx_fir_taps_list invalid format for port {}: {}".format(port_id, tx_fir_taps))
                ...
                logging.info("Port serdes expected attributes in COUNTERS_DB PORT_PHY_ATTR table verified for port {}: tx_fir_taps={}, rx_vga={}".format(
                    port_id, tx_fir_taps[:50], rx_vga))

            except SonicDbKeyNotFound:
                pytest.fail("Port serdes expected attributes not found in COUNTERS_DB PORT_PHY_ATTR table for port {}".format(port_id))

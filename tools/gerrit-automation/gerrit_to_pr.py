...
class GerritToPRConverter:
    ...
    def __init__(self, gerrit_url: str, username: str, password: str, github_token: str):
        ...
        self.component_map = {
            ...
            'cfgmgr/': 'sonic-swss', 
            'syncd/': 'sonic-swss',
            'portsyncd/': 'sonic-swss',
            'neighsyncd/': 'sonic-swss',
            'fpmsyncd/': 'sonic-swss',
            'scripts/sonic-kdump-config': 'sonic-utilities',
            'sonic-utilities': 'sonic-utilities',
            'scripts/': 'sonic-utilities',
            'utilities/': 'sonic-utilities',
            'build_image.sh': 'sonic-buildimage',
            'rules/config': 'sonic-buildimage',
    ...
    def gerrit_get(self, endpoint: str) -> Dict:
        ...
        print(f"DEBUG: Making request to {url} with user: {self.username}")
        ...
        if response.status_code == 401:
            print(f"DEBUG: Authentication failed. Check if credentials are correct.")
            print(f"DEBUG: Try this curl command to verify:")
    ...
    def extract_upstreamable_patches(self, series_content: str) -> List[str]:
        ...
        print(f"DEBUG: Analyzing series file with {len(lines)} lines (treating all as upstreamable)")
        ...
        for i, line in enumerate(lines):
            ...
            if line_stripped and not line_stripped.startswith('#'):
                if line_stripped.endswith('.patch'):
                    ...
                    print(f"DEBUG: Found patch: {line_stripped}")
    ...
    def extract_subject_from_patch(self, patch_content: str) -> str:
        ...
        return "SONiC patch from Gerrit"
...
def main():
    ...
    if args.config and os.path.exists(args.config):
        ...
        print(f"DEBUG: Loaded config - URL: {gerrit_url}, User: {gerrit_user}, Pass length: {len(gerrit_pass) if gerrit_pass else 0}")
    ...
    try:
        ...
        if pr_urls:
            print(f"\n✅ Successfully created {len(pr_urls)} PRs:")
            for url in pr_urls:
                print(f"  - {url}")
            
            # Post Gerrit comment with PR links for internal team
            self.post_gerrit_comment(change_id, pr_urls)
        else:
            print("\n⚠️  No PRs created - no upstreamable patches found")
